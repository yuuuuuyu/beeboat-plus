...
class BTPViewContext extends base_view_context_1.default {
    constructor(vueInstance, viewId, viewModelId, parentViewContext) {
        ...
        if (this.viewModelId) {
            utils_ex_1.default.getAppManager()
            ...
            console.log('无法获取参数', application_1.default.getInstance().getRouter().currentRoute);
        }
    }
    /**
    ...
    buildProps(data) {
        const componentList = utils_ex_1.default.getAppManager().parseComponentList(data);
    ...
    buildEvents(components) {
        ...
        const componentList = utils_ex_1.default.getAppManager().parseComponentList(components);
        componentList.forEach(item => {
            item.events = {};
                ...
                Object.keys(item.actions).forEach(eventName => {
                    ...
                    if (action.propEvent) {
                        item.props.propEvents[eventName] = (p1, p2, p3, p4, p5, p6, p7) => {
                            const executor = new logic_executor_1.default(viewContext, item, action, eventName, [p1, p2, p3, p4, p5, p6, p7]);
...
exports.default = BTPViewContext;
