...
export function calculationHoverColor(color: string) {
...
export function calculationClickColor(color: string) {
    const rgb = hexToRGB(color)
    const hsb = RGBToHSB(rgb[0], rgb[1], rgb[2])
...
export function calculationColor(color: string, h = 0, s = 0, b = 0) {
    const rgb = hexToRGB(color)
    const hsb = RGBToHSB(rgb[0], rgb[1], rgb[2])
    // 10-17 s和b 值不能大于100
    const rgbNew = HSBToRGB(
        hsb[0] + h,
        hsb[1] + s > 100 ? 100 : hsb[1] + s,
        hsb[2] + b > 100 ? 100 : hsb[2] + b,
    )
    const colorNew = RGBToHex(rgbNew[0], rgbNew[1], rgbNew[2])
    return colorNew
}
