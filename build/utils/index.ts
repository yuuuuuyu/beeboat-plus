...
import { definedReplaceAll } from './replace-all'

// 自定义每个task的name
export const withTaskName = <T>(name: string, fn: T) => Object.assign(fn, { displayName: name })

// 在node中开启一个子进程来运行脚本
export const run = async (command: string, path: string = projectRoot) => {
...
export const pathRewriter = format => {
    return (id: string) => {
        id = definedReplaceAll(id, '@beeboat', `beeboat-plus/${format}`)
        return id
    }
}
