<template>
    <el-popover trigger="hover" :width="370">
        <template #reference>
        ...
        <template #default>
            <div class="bt-table-col-setting">
                ...
                <div class="field-footer">
                    <bt-button type="blank" @click="onColReset"> 取 消 </bt-button>
                    <bt-button type="primary" @click="onColSave"> 保 存 </bt-button>
                </div>
            </div>
        </template>
    </el-popover>
...
const searchName = ref('') // 搜索项
let dataList = ref([]) as any // 列数据
let originList = [...dataList.value] // 列数据
watch(
    () => props.colSetting,
    () => {
        dataList.value = clone(props.colSetting)
        originList = [...clone(dataList.value)]
    },
    {
        deep: true,
        immediate: true,
...
const dragend = _val => {
    if (oldVal.value != newOld.value) {
        let oldIndex = dataList.value.findIndex(i => i.id == oldVal.value)
        let newIndex = dataList.value.findIndex(i => i.id == newOld.value)
        let newItems = [...dataList.value]
        // 删除老的节点
        ...
        // 对originList做拖拽处理
        let oldIndex1 = originList.findIndex(i => i.id == oldVal.value)
        let newIndex1 = originList.findIndex(i => i.id == newOld.value)
        let newItems1 = [...originList]
        // 删除老的节点
        originList.splice(oldIndex1, 1)
        // 增加新的节点
        originList.splice(
            newIndex1,
            0,
            newItems1.find(i => i.id == oldVal.value),
        )
...
const onInput = val => {
    ...
    else dataList.value = clone(originList)
}
const onCheckChange = val => {
    if (isActive.value.toString() != '-1') {
        ...
        originList.filter(item => {
            if (item.id == isActive.value) {
                item.showValue = val == true
            }
        })
    }
}
const onFixed = (index, val) => {
    dataList.value[index].fixed = val && val.trim().length > 0 ? '' : 'left'
...
// 操作列保存
const onColSave = () => {
    dataList.value = clone(originList)
    // 操作列同步保存操作
    emits('fresh', { columns: dataList.value, type: 'save' })
}
// 操作列重置操作
const onColReset = () => {
    dataList.value = clone(props.colSetting)
    emits('fresh', { columns: props.colSetting, type: 'reset' })
}
</script>
