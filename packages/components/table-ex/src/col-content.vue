<template>
    <!-- editable 表示当前列可编辑 -->
    <template v-if="item?.editProps?.enable">
        <inner-edit-item
            v-if="props.currentRow.includes(props.scope.$index)"
            :row="props.scope.row"
            :column="item"
        />
        <template v-else-if="item?.dictId">
    ...
    <template v-else>
        ...
        <template v-else-if="item?.searchProps?.dataSourceProps">
            {{ dataSourceShowe(scope.row, item, props.tableSrc) }}
            {{
                scope.row[`${item.prop!}show`]
                    ? scope.row[`${item.prop!}show`]
                    : scope.row[`${item.prop!}`]
            }}
        </template>
...
import InnerEditItem from './inner-edit-item.vue'
import { BtUseAppStore } from '@beeboat/core'
const props = defineProps<{ item: any; scope: any; currentRow: any; tableSrc:any}>()
