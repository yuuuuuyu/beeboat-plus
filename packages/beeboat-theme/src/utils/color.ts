...
export function extendHex(shortHex: string) {
    return `#${shortHex}`
...
export function toRGBObject(rgbStr: any) {
...
//将 rgb()颜色字符串转换为数组
export function toRGBArray(rgbStr: any) {
    return rgbStr.match(/\d+/g).map(Number)
}

// 将 RGB 颜色元组转换为 HSB 格式。
export function RGBToHSB(r: any, g: any, b: any) {
    r /= 255
    g /= 255
    b /= 255
...
export function HSBToRGB(h: any, s: any, b: any) {
    s /= 100
    b /= 100
...
//更改hsl()颜色字符串的亮度值。
export function changeLightness(delta: any, hslStr: any) {
...
// 将hsl()颜色字符串转换为值数组。
export function toHSLArray(hslStr: any) {
